...
def help_msg(update, context):
    update.message.reply_text("help um illa oru koppum illa ezheech podey")
...
def username(update, context):
    ...
    if not re.compile(mediaregpat).search(query):
        msg = update.message.reply_text("Wait cheyy My#@...")
        ...
        try:
            ...
            msg.edit_text("Inna pidichoo")
            time.sleep(5)
        except Exception as e:
            print(format_exc())
            msg.edit_text("Nokkeem kandum okke username adikkeda potta nee paranja username thettaanu")
    else:
        update.message.reply_html("Eduthond poda ooole🤭 photo yum media yum ayacha njan endh thenga cheyyanaa 🤧")
